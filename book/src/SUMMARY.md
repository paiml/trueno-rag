# Summary

[Introduction](./introduction.md)

# Getting Started

- [Quick Start](./quickstart.md)
- [Installation](./installation.md)

# Core Concepts

- [RAG Pipeline Overview](./concepts/pipeline.md)
- [Document Chunking](./concepts/chunking.md)
- [Embeddings](./concepts/embeddings.md)
- [Retrieval](./concepts/retrieval.md)
- [Fusion Strategies](./concepts/fusion.md)
- [Reranking](./concepts/reranking.md)

# Advanced Topics

- [Semantic Chunking](./advanced/semantic-chunking.md)
- [Query Preprocessing](./advanced/preprocessing.md)
- [Evaluation Metrics](./advanced/metrics.md)
- [Evaluation Pipeline](./advanced/eval-pipeline.md)
- [Performance Tuning](./advanced/performance.md)
- [Semantic Embeddings (GH-1)](./advanced/semantic-embeddings.md)
- [Index Compression (GH-2)](./advanced/compression.md)
- [Nemotron Embeddings (GH-3)](./advanced/nemotron-embeddings.md)

# CLI Reference

- [Command Reference](./cli/reference.md)

# API Reference

- [Pipeline Builder](./api/pipeline.md)
- [Chunkers](./api/chunkers.md)
- [Embedders](./api/embedders.md)
- [Retrievers](./api/retrievers.md)
- [Rerankers](./api/rerankers.md)

# Examples

- [Basic RAG Pipeline](./examples/basic.md)
- [Custom Chunking](./examples/custom-chunking.md)
- [Hybrid Search](./examples/hybrid-search.md)
- [Metrics Evaluation](./examples/metrics-evaluation.md)
- [Eval Hybrid](./examples/eval-hybrid.md)
- [Compressed Index](./examples/compressed-index.md)
- [Semantic Embeddings](./examples/semantic-embeddings.md)
- [Nemotron Embeddings](./examples/nemotron-embeddings.md)
